<div class="modal-header">
    ...
    <h4 class="modal-title"><?php foreach ($monit as $dat){ echo ''.$dat->apellidos.' '.$dat->nombres.'';} ?></h4>
</div>
<div class="modal-body">
    <form id="formulariocambiardatos" method="post" action="<?php echo base_url(); ?>mmonitores/modificardatos">
        <div class="form-group">
            <div class="col-md-4"><label for="inputdefault">Código</label>
                <input type="text" id="txtcodigoviejho" name="txtcodigoviejho"
                    value="<?php foreach ($monit as $dat){ echo $dat->codigo;} ?>" style="display:none;">
                <input class="form-control" id="txtcodigoactu" name="txtcodigoactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->codigo;} ?>"></div>
            <div class="col-md-4"><label for="inputdefault">Nombres</label>
                <input class="form-control" id="txtnbombreacut" name="txtnbombreacut" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->nombres;} ?>"></div>
            <div class="col-md-4"><label for="inputdefault">Apellidos</label>
                <input class="form-control" id="textapealisod" name="textapealisod" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->apellidos;} ?>"></div>

        </div>
        <div class="form-group">
            <div class="col-md-6"><label for="inputdefault">Correo</label>
                <input class="form-control" id="txtcorroactu" name="txtcorroactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->correo;} ?>"></div>
            <div class="col-md-6">
                <label for="inputdefault">Telefono</label>
                <input class="form-control" id="txtytelactu" name="txtytelactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->tel;} ?>">
            </div>
        </div>

        <div class="form-group">
            <div class="col-md-4"><label for="inputdefault">Dirección</label>
                <input class="form-control" id="txtdireccionactu" name="txtdireccionactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->direccion;} ?>"></div>
            <div class="col-md-4"><label for="inputdefault">Barrio</label>
                <input class="form-control" id="txtbarrioactu" name="txtbarrioactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->barrio;} ?>"></div>
            <div class="col-md-4"><label for="inputdefault">Ciudad</label>
                <input class="form-control" id="txtciaudadactu" name="txtciaudadactu" type="text"
                    value="<?php foreach ($monit as $dat){ echo $dat->ciudad;} ?>">
        ...
        <div class="form-group">
            <div class="col-md-4">
                ...
                <select class="form-control" name="txtcarrera" id="txtcarrera">
                    ...
                    <?php foreach($programas as $prgns){ 
                        foreach($monit as $duser){
                            if($duser->programas == $prgns->nombre){
                                echo '<option value="'.$prgns->id.'" selected>'.$prgns->nombre.'</option>'; 
                            }else{
                                echo '<option value="'.$prgns->id.'">'.$prgns->nombre.'</option>';  
                            }
            ...
            <div class="col-md-4">
                ...
                <select class="form-control" name="txtsemestre" id="txtsemestre">
                    <?php foreach($monit as $duser){ echo '<option selected>'.$duser->semestre.'</option>'; } ?>
            ...
            <div class="col-md-4">
                ...
                <select class="form-control" name="txttopm" id="txttopm">
        ...
        <div class="form-group">
            <div class="col-md-6">
                ...
                <select class="form-control" name="txtdependencia" id="txtdependencia" title="Procesos,GTI,Humanas">
                    <?php foreach($dependencias as $depen){ 
                        foreach($monit as $duser){
                            if($duser->dependencia == $depen->nombre){
                                echo '<option value="'.$depen->id.'" selected>'.$depen->nombre.'</option>';
                            }else{
                                echo '<option value="'.$depen->id.'">'.$depen->nombre.'</option>';
                            }
...
-------------------------------------------------------------->
                <span id="editfor" class="label label-danger">Editar</span><span id="cancelarfor"
                    class="label label-info">Cancelar</span>
                <input type="text" class="form-control" id="materiasb" name="materiasb"
                    value="<?php foreach ($monit as $dat){ echo $dat->areasesorias;} ?>" readonly>
                <select class="selectpicker" multiple title="Seleccione las areas academicas..." data-live-search="true"
                    data-selected-text-format="count > 5" id="areasfortaleza">
...
-------------------------------------------------------------->
        ...
        <input type="number" class="form-control" name="txthoraasignadas" id="txthoraasignadas" value="<?php foreach ($monit as $dat){ echo $dat->horaasignadas;} ?>">
        </div>
        <p>&nbsp;</p>
        <div class="form-group">
            <div class="col-md-6">
                <button type="button" class="btn btn-default form-control" data-dismiss="modal">Cancelar</button></div>
            <div class="col-md-6"><button type="submit" class="btn btn-success form-control">Cambiar
        ...
        $('#formulariocambiardatos').on('submit', function (e) {
            ...
            $.ajax({
                ...
                complete: function (data) {
                    swal("Listo", "modifico los datos correctamente", "success");
                    location.reload();
                }
            });
